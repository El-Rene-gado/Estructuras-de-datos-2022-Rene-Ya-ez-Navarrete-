...
// para que sea min heap bastaria con voltear la primera sentencia if en el metodo max(el que recibe parametros), ya que recorre el heap por completo
...
class ColaBinomial {
	...
	private  ColaBinomial BinomialHeapMerge(ColaBinomial H1, ColaBinomial H2) { 
        ...
        public int max(){ //funcion sin recibir parametros
        return max(head,0);
                
        }
        
        public int max(NodoCB a, int b){
            int max=b; // entero maximo
            
            NodoCB x=a;
            if(max<x.key)   max=x.key;          
            
            if(x.sibling!=null)max=max(x.sibling,max); // recorre los hermanos para ver cual contiene el mayor
            if(x.child!=null)max=max(x.child,max);  // recorre los hijos de aquel hermano para ver cual es el mayor
